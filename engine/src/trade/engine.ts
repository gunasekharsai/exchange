...
import { Fill, Order, Orderbook } from "./orderbokk";
...
export class Engine {
        ...
        switch (message.type) {
            ...
            case CANCEL_ORDER:
                try {
                    ...
                    const order = cancelOrderbook.asks.find((o:any) => o.orderId === orderId) || cancelOrderbook.bids.find(o => o.orderId === orderId);
                    if (!order) {
    ...
    publisWsDepthUpdates(fills: Fill[], price: string, side: "buy" | "sell", market: string) {
        ...
        const depth = orderbook.getDepth();
        if (side === "buy") {
            const updatedAsks = depth?.asks.filter((x:any) => fills.map(f => f.price).includes(x[0].toString()));
